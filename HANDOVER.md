...
heat / screamer hidden until the outcome beat; count-ups on impact). Reveal mode is a player preference
(`matchHighlightMode`, toggle in the live controls): **manual/"Tap" is the default** (tap to step one
beat at a time), plus "auto" (timed) and a "Skip"; reduced-motion = instant. **Presentation only: zero new persisted state, no SAVE bump, season-lab
